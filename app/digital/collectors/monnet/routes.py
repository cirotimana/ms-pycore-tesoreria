from app.digital.collectors.monnet.tasks import task_process_monnet, task_process_updated_monnet
...
@router.get("/execute-getmonnet")
@redis_lock("monnet-process")
def execute_get_monnet(from_date: str = None, to_date: str = None):
    try:
        task = task_process_monnet.delay(from_date, to_date)
        logger.info(LOG_TASK_STARTED.format(task_name="task_process_monnet", task_id=task.id))
...
@router.get("/execute-updated-monnet")
@redis_lock("monnet-process")
def execute_updated_monnet():
    try:
        task = task_process_updated_monnet.delay()
        logger.info(LOG_TASK_STARTED.format(task_name="task_process_updated_monnet", task_id=task.id))
