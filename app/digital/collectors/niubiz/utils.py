...
class TokenCacheNiubiz:
    ...
    async def get_token(self, force_refresh=False, type=2):
        async with self.lock:
            ...
            print(f"[INFO] Obteniendo nuevo token Niubiz tipo {type}...")
            if type == 1:
                self.token = await get_token_niubiz_1()
            else:
                self.token = await get_token_niubiz_2()
...
async def get_token_niubiz_2(max_login_attempts=5):
    ...
    try:
        async with async_playwright() as p:
            print("[INFO] Lanzando navegador para Niubiz")
            ...
            async def intentar_login_niubiz():
                try:
                    ...
                    for selector in username_selectors:
                        try:
                            ...
                            await page.fill(selector, Config.USER_NAME_NIUBIZ)
                    ...
                    for selector in password_selectors:
                        try:
                            ...
                            await page.wait_for_selector(selector, state='visible', timeout=5000)
                            await page.fill(selector, Config.PASSWORD_NIUBIZ)
                    ...
                    print(f"[ERROR] Error en intentar_login_niubiz: {e}")
                    return False
            ...
            while login_attempt < max_login_attempts:
                ...
                login_success = await intentar_login_niubiz()
...
async def get_download_data_niubiz_async(token, from_date, to_date):
...
def process_zip_file_niubiz(zip_content, timestamp):
    downloaded_files = []
        ...
        with zipfile.ZipFile(BytesIO(zip_content)) as zip_file:
            ...
            for file_name in file_list:
                if file_name.endswith(('.csv', '.xlsx', '.xls')):
                    ...
                    output_key = f"digital/collectors/niubiz/input/{base_name}_{timestamp}{file_ext}"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if filename.endswith('.zip'):
                    ...
                    extracted_files = process_zip_file_niubiz(content, current_time)
                    downloaded_files.extend(extracted_files)
                else:
                    # Si no es ZIP, guardar directamente
                    output_key = f"digital/collectors/niubiz/input/{filename.replace('.zip', '')}_{current_time}"
...
async def get_niubiz_reports_async(token, from_date, to_date, max_wait_minutes=30):
    print("[INFO] Iniciando proceso completo de descarga de reportes Niubiz")
    
    # Paso 1: Solicitar reportes
    id_list, updated_token = await get_download_data_niubiz_async(token, from_date, to_date)
...
async def get_data_main_async_2(from_date, to_date):
    try:
        print("[INFO] Iniciando captura de token Niubiz")
        token = await token_cache_niubiz.get_token()
        
        if token:
            print("[INFO] Trayendo datos de Niubiz")
            downloaded_files = await get_niubiz_reports_async(token, from_date, to_date)
            print(f"Archivos descargados: {downloaded_files}")
            return downloaded_files
        else:
            print("[ALERTA] No se pudo obtener el token de Niubiz.")
            
    except Exception as e:
        print(f"[✖] Error en obtener la data de Niubiz: {e}")
    

def get_data_main_2(from_date, to_date):
    print(f"[INICIO] Ejecutando Niubiz para {from_date} a {to_date}")
    result = asyncio.run(get_data_main_async_2(from_date, to_date))
    print(f"[FIN] Proceso Niubiz completado {result}")
    return result
                    ...
                    if response.status_code == 200:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            if new_token:
                                ...
                                print("[ALERTA] No se pudo renovar el token Niubiz.")
