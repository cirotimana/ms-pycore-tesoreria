from app.digital.collectors.kashio.liquidations.tasks import task_process_kashio_liq
...
@redis_lock("kashio-process-liq")
def execute_liq_kashio(from_date: str = None, to_date: str = None):
    try:
        task = task_process_kashio_liq.delay(from_date, to_date)
        logger.info(LOG_TASK_STARTED.format(task_name="task_process_kashio_liq", task_id=task.id))
        ...
        logger.error(f"Error al iniciar tarea Kashio Liquidations: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
