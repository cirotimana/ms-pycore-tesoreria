from app.digital.collectors.tupay.tasks import task_process_tupay, task_process_updated_tupay
...
@router.get("/execute-gettupay")
@redis_lock("tupay-process")
def execute_get_tupay(from_date: str = None, to_date: str = None):
    try:
        task = task_process_tupay.delay(from_date, to_date)
        logger.info(LOG_TASK_STARTED.format(task_name="task_process_tupay", task_id=task.id))
...
@router.get("/execute-updated-tupay")
@redis_lock("tupay-process")
def execute_updated_tupay():
    try:
        task = task_process_updated_tupay.delay()
        logger.info(LOG_TASK_STARTED.format(task_name="task_process_updated_tupay", task_id=task.id))
