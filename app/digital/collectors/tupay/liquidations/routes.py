from app.digital.collectors.tupay.liquidations.tasks import task_process_tupay_liq
...
@router.get("/execute-liqtupay")
@redis_lock("tupay-process-liq")
def execute_liq_tupay(from_date: str = None, to_date: str = None):
    try:
        task = task_process_tupay_liq.delay(from_date, to_date)
        logger.info(LOG_TASK_STARTED.format(task_name="task_process_tupay_liq", task_id=task.id))
        ...
        logger.error(f"Error al iniciar tarea Tupay Liquidations: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
