from app.digital.collectors.kushki.tasks import task_process_kushki, task_process_updated_kushki
...
@router.get("/execute-getkushki")
@redis_lock("kushki-process")
def execute_get_kushki(from_date: str = None, to_date: str = None):
    try:
        task = task_process_kushki.delay(from_date, to_date)
        logger.info(LOG_TASK_STARTED.format(task_name="task_process_kushki", task_id=task.id))
...
@router.get("/execute-updated-kushki")
@redis_lock("kushki-process")
def execute_updated_kushki():
    try:
        task = task_process_updated_kushki.delay()
        logger.info(LOG_TASK_STARTED.format(task_name="task_process_updated_kushki", task_id=task.id))
