from app.digital.DNIcorrelatives.tasks import task_process_dni_correlatives
...
@router.get("/execute-dnicorrelatives")
@redis_lock("dni-correlatives-process")
def execute_dnicorrelatives():
    try:
        # Enviar tarea a la cola de Celery
        task = task_process_dni_correlatives.delay()
        
        logger.info(LOG_TASK_STARTED.format(
            task_name="task_process_dni_correlatives",
            task_id=task.id
        ))
